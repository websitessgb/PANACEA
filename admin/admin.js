...
function money(value,currency='CUP'){
 return `${new Intl.NumberFormat('es-CU').format(Number(value)||0)} ${currency}`;
}

function formatDate(value){
 const d=new Date(value);
 if(Number.isNaN(d.getTime()))return value||'';
 return new Intl.DateTimeFormat('es-CU',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}).format(d);
}
...
function statusLabel(status){
 return {pending:'Pendiente',confirmed:'Confirmado',invoiced:'Facturado',delivered:'Entregado',cancelled:'Cancelado'}[status]||'Pendiente';
}
...
function parseCustomer(text){
 const m=text.match(/(?:👤\s*)?Cliente\s*:\s*(.+)/i);
 return m?m[1].trim():'';
}

function parsePhone(text){
 const m=text.match(/(?:📞\s*)?Teléfono\s*:\s*([0-9+\-\s()]+)/i);
 return m?m[1].trim().replace(/[^\d+]/g,''):'';
}

function getProductsSection(text){
 const start=text.search(/📦\s*PRODUCTOS/i);
 if(start===-1)return '';
 const after=text.slice(start);
 const separator=after.indexOf('━━━━━━━━━━━━━━');
 return separator!==-1?after.slice(0,separator):after;
}
...
function parseProducts(text){
 const section=getProductsSection(text);
 if(!section)return [];
 return section.split('\n').map(x=>x.trim()).filter(x=>x.startsWith('•')).map(line=>{
  const clean=line.replace(/^•\s*/,'').trim();
  const m=clean.match(/^(.+?)\s+—\s+(\d+)\s+(.+?)\s+×\s+([\d.,]+)\s+(CUP|USD)\s+=\s+([\d.,]+)\s+(CUP|USD)$/i);
  if(!m)return null;
  return {
   name:m[1].trim(),
   quantity:Number(m[2]),
...
function parseTotals(text){
 const cup=text.match(/TOTAL\s+CUP\s*:\s*([\d.,]+)\s*CUP/i);
 const usd=text.match(/TOTAL\s+USD\s*:\s*([\d.,]+)\s*USD/i);
 return {
...
function nextOrderNumber(date=new Date()){
 ...
 const prefix=`PAN-${year}-S${week}-`;
 let highest=0;
 orders.filter(o=>String(o.orderNumber||'').startsWith(prefix)).forEach(o=>{
...
function parseWhatsAppOrder(text){
 ...
 const phone=parsePhone(cleanText);
 if(!phone)throw new Error('No pude encontrar el teléfono del cliente.');
 const products=parseProducts(cleanText);
 if(!products.length)throw new Error('No pude interpretar los productos. Asegúrate de pegar el mensaje completo de WhatsApp.');
 ...
 return {
  ...
  week:getISOWeek(now),
  year:getISOYear(now),
  customer,phone,products,totals,
  status:'pending',
  source:'whatsapp',
...
function formatTotals(totals){
 const result=[];
 if(Number(totals?.CUP)>0)result.push(money(totals.CUP,'CUP'));
 if(Number(totals?.USD)>0)result.push(money(totals.USD,'USD'));
...
function filteredOrders(){
 ...
 return orders.filter(o=>{
  ...
  if(!q)return true;
  const searchable=[o.orderNumber,o.customer,o.phone,...(o.products||[]).map(p=>p.name)].join(' ').toLowerCase();
...
function renderOrders(){
 ...
 ordersList.innerHTML=list.map(order=>{
  const total=formatTotals(order.totals);
  const productNames=(order.products||[]).map(p=>`${p.quantity} ${p.name}`).join(' · ');
  return `<article class="order-card" data-order-id="${escapeHTML(order.orderNumber)}">
   <div><div class="order-number">${escapeHTML(order.orderNumber)}</div><div class="order-date">${escapeHTML(formatDate(order.createdAt))}</div></div>
   <div><div class="order-customer">${escapeHTML(order.customer)}</div><div class="order-phone">${escapeHTML(order.phone)}</div><div class="order-summary">${escapeHTML(productNames)}</div></div>
   <div class="order-right"><div class="order-total">${escapeHTML(total)}</div><span class="status-badge ${statusClass(order.status)}">${escapeHTML(statusLabel(order.status))}</span><br><button type="button" class="order-open-btn" data-open-order="${escapeHTML(order.orderNumber)}">Ver pedido →</button></div>
  </article>`;
 }).join('');
...
function renderStats(){
 ...
 const cup=orders.reduce((s,o)=>s+Number(o.totals?.CUP||0),0);
 document.getElementById('statCup').textContent=money(cup,'CUP');
}
...
function openOrder(orderNumber){
 ...
 const products=(order.products||[]).map(p=>`
  <div class="detail-product">
   <div><strong>${escapeHTML(p.name)}</strong><br><small>${escapeHTML(p.presentation)}</small></div>
   <div>${escapeHTML(String(p.quantity))}</div>
   <div>${escapeHTML(money(p.total,p.currency))}</div>
  </div>`).join('');

 orderDetail.innerHTML=`
  <div class="order-detail">
   <button type="button" class="dialog-x" data-close-order aria-label="Cerrar">×</button>
   <div class="detail-top">
    <div><div class="detail-number">${escapeHTML(order.orderNumber)}</div><div class="detail-date">${escapeHTML(formatDate(order.createdAt))} · Semana ${escapeHTML(String(order.week))}</div></div>
    <span class="status-badge ${statusClass(order.status)}">${escapeHTML(statusLabel(order.status))}</span>
   </div>
   <div class="detail-customer">
    <div class="detail-box"><small>CLIENTE</small><strong>${escapeHTML(order.customer)}</strong></div>
    <div class="detail-box"><small>TELÉFONO</small><strong>${escapeHTML(order.phone)}</strong></div>
   </div>
   <div class="detail-products"><h3>Productos</h3>${products}</div>
   <div class="detail-totals"><div class="detail-total">${escapeHTML(formatTotals(order.totals))}</div></div>
   <div class="detail-actions">
    <button type="button" data-send-self="${escapeHTML(order.orderNumber)}">📱 Enviar a mi WhatsApp</button>
    <button type="button" data-send-client="${escapeHTML(order.orderNumber)}">👤 Enviar al cliente</button>
    <button type="button" data-copy-order="${escapeHTML(order.orderNumber)}">📋 Copiar pedido</button>
    <button type="button" data-status="${escapeHTML(order.orderNumber)}">🔄 Cambiar estado</button>
    <button type="button" class="danger" data-delete-order="${escapeHTML(order.orderNumber)}">🗑️ Eliminar</button>
   </div>
  </div>`;
...
function changeStatus(orderNumber){
 ...
 select.innerHTML=options.map(([value,label])=>`<option value="${value}" ${order.status===value?'selected':''}>${label}</option>`).join('');
 select.style.cssText='margin-top:10px;padding:10px;width:100%;font-size:16px;';

 const current=document.querySelector('[data-status]');
 if(current){
...
function buildOrderMessage(order){
 ...
 let totals='';
 if(Number(order.totals?.CUP)>0)totals+=`💰 TOTAL CUP: ${money(order.totals.CUP,'CUP')}\n`;
...
function normalizeCubanPhone(phone){
 ...
 if(value.startsWith('53'))return value;
 return value.length===8?`53${value}`:value;
}

function sendToSelf(orderNumber){
 const order=orders.find(o=>o.orderNumber===orderNumber);
 if(!order)return;
 window.open(`https://wa.me/${PANACEA_WHATSAPP}?text=${encodeURIComponent(buildOrderMessage(order))}`,'_blank');
...
function sendToClient(orderNumber){
 const order=orders.find(o=>o.orderNumber===orderNumber);
 if(!order)return;
 const phone=normalizeCubanPhone(order.phone);
 if(!phone)return showToast('El teléfono del cliente no es válido.');
