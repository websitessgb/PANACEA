...
flexibleDialog=document.getElementById('flexibleDialog'),
orderDialog=document.getElementById('orderDialog'),
whatsappMessage=document.getElementById('whatsappMessage'),
flexibleMessage=document.getElementById('flexibleMessage'),
importPreview=document.getElementById('importPreview'),
flexiblePreview=document.getElementById('flexiblePreview'),
parseError=document.getElementById('parseError'),
flexibleError=document.getElementById('flexibleError'),
...
function openNewOrderChooser(){ if(newOrderDialog) newOrderDialog.showModal(); }
function closeNewOrderChooser(){ if(newOrderDialog?.open) newOrderDialog.close(); }
function openWhatsAppImportFromChooser(){ closeNewOrderChooser(); openImport(); }

function normalizeFlexibleText(v){ return String(v||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,' ').trim(); }
function flexibleCustomer(t){ const m=String(t||'').match(/(?:^|\n)\s*(?:nombre\s+del\s+cliente|nombre|cliente)\s*:\s*(.+)\s*$/im); return m?m[1].trim():''; }
...
function parseFlexibleOrder(text){
...
function parseCustomer(t){
  ...
  // El emoji 👤 puede desaparecer o convertirse en "?"
  // cuando el pedido pasa por WhatsApp Web/PC.
  // El dato importante es la etiqueta "Cliente:".
  const m=t.match(
    /^\s*(?:[?👤]\s*)?(?:\*\s*)?Cliente(?:\s*\*)?\s*:\s*(.+)\s*$/im
  );
...
function parsePhone(t){
  ...
  // Igual que con Cliente, no dependemos del emoji 📞.
  const m=t.match(
    /^\s*(?:[?📞]\s*)?(?:\*\s*)?Teléfono(?:\s*\*)?\s*:\s*([0-9+\-\s()]+)\s*$/im
  );
...
function getProductsSection(t){
  ...
  // NO depender del emoji 📦. En PC/WhatsApp Web puede
  // convertirse en "?" o desaparecer. Aceptamos, por ejemplo:
  // 📦 PRODUCTOS, ? PRODUCTOS, PRODUCTOS, *PRODUCTOS:*
  const m=t.match(
    /^\s*(?:[?📦]\s*)?(?:\*\s*)?PRODUCTOS(?:\s*\*)?\s*:?.*$/im
  );

  if(!m)return'';

  const start=m.index;
  const a=t.slice(start);
...
function parseProducts(t){
  ...
  return s.split('\n')
    .map(x=>x.trim())
    // No exigimos que exista el punto/viñeta delante del producto.
    // WhatsApp Web puede eliminarlo, cambiarlo o convertirlo en otro símbolo.
    // Lo importante es reconocer la estructura del producto a partir de
    // nombre + cantidad + presentación + precio + moneda + total.
    .filter(x=>x && !/^PRODUCTOS\b/i.test(x))
    .map(line=>{

      const c=line
        // Quita viñetas o caracteres que WhatsApp pueda poner al inicio.
        .replace(/^[•·▪▫◦‣►▸*-]+\s*/,'')
        .replace(/^\?\s+/,'')
        .trim();
      ...
      const m=c.match(
        /^(.*?)\s+[—–-]\s+(\d+)\s+(.+?)\s+[×xX]\s+([\d.,]+)\s+(CUP|USD)\s*[=≡:]\s*([\d.,]+)\s+(CUP|USD)\s*$/i
      );
...
function renderRestoreAnalysis(analysis){
  ...
  const intro=document.getElementById('restoreIntro');
...
function renderRestoreSummary(){
  ...
  box.innerHTML=`
    <div class="restore-analysis-header">
      <p class="admin-eyebrow">SEGURIDAD</p>
      <h2>Análisis del respaldo</h2>
      <p>Revisa el resultado antes de aplicar cualquier modificación.</p>
    </div>
    ...
    <div class="restore-analysis-actions">
      ${changed?`
...
function renderRestoreConflict(index){
  ...
  box.innerHTML=`
    <div class="restore-analysis-header restore-analysis-header-compact">
      <p class="admin-eyebrow">SEGURIDAD</p>
      <h2>Análisis del respaldo</h2>
    </div>
...
document.getElementById('closeNewOrderBtn').addEventListener('click',closeNewOrderChooser);
document.getElementById('cancelNewOrderBtn').addEventListener('click',closeNewOrderChooser);
document.getElementById('chooseWhatsAppBtn').addEventListener('click',openWhatsAppImportFromChooser);
document.getElementById('chooseFlexibleBtn').addEventListener('click',()=>{closeNewOrderChooser();openFlexibleImport();});
...
document.getElementById('closeFlexibleBtn').addEventListener('click',closeFlexibleImport);
document.getElementById('cancelFlexibleBtn').addEventListener('click',closeFlexibleImport);
previewFlexibleBtn.addEventListener('click',previewFlexibleImport);
document.getElementById('flexibleForm').addEventListener('submit',e=>{e.preventDefault();createFlexibleOrder();});
...
/* ACCESO RÁPIDO A RESPALDOS */
document.getElementById('backupToggle').addEventListener('click',()=>{
  document.getElementById('respaldos').scrollIntoView({
    behavior:'smooth',
    block:'start'
  });
});
