...
function parseCustomer(t){
  ...
  /* Los emojis son decorativos: la palabra "Cliente" es la clave. */
  const m=t.match(
    /(?:^|\n)\s*[^A-Za-zÁÉÍÓÚáéíóúÑñ0-9\n]*Cliente\s*:\s*(.+)\s*$/im
  );
...
function parsePhone(t){
  ...
  /* Los emojis son decorativos: la palabra "Teléfono" es la clave. */
  const m=t.match(
    /(?:^|\n)\s*[^A-Za-zÁÉÍÓÚáéíóúÑñ0-9\n]*Teléfono\s*:\s*([0-9+\-\s()]+)/im
  );
...
function getProductsSection(t){
  ...
  /*
    NO dependemos del emoji 📦.
    WhatsApp Web puede conservarlo, eliminarlo o convertirlo en ??.
    La palabra PRODUCTOS es la clave estable del formato.
  */
  const m=t.match(
    /(?:^|\n)\s*[^A-Za-zÁÉÍÓÚáéíóúÑñ0-9\n]*PRODUCTOS\s*:?/im
  );
  ...
  /* Detenernos antes de la línea separadora o del TOTAL. */
  const x=a.search(
    /(?:^|\n)\s*(?:[-_=─━]{4,}|TOTAL\s+(?:CUP|USD)\s*:)/i
  );
...
function parseProducts(t){
  ...
  return s.split('\n')
    .map(line=>{
      ...
      /*
        Las viñetas también son decorativas.
        Aceptamos producto con •, ·, -, *, ?? u otros símbolos delante,
        e incluso sin ningún símbolo.
      */
      const c=String(line||'')
        .trim()
      ...
      /* El producto se reconoce por su estructura, no por la viñeta. */
      const m=c.match(
        /^(.+?)\s+—\s+(\d+)\s+(.+?)\s+×\s+([\d.,]+)\s+(CUP|USD)\s+=\s+([\d.,]+)\s+(CUP|USD)\s*$/i
      );
