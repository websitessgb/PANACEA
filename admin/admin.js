...
function nextOrderNumber(d=new Date()){
  ...
  orders.filter(o=>String(o.orderNumber||'').startsWith(prefix))
    .forEach(o=>{
...
function filteredOrders(){
  ...
  return orders.filter(o=>{
    ...
    const s=[
      o.orderNumber,o.customer,o.phone,
...
function renderOrders(){
  ...
  document.querySelectorAll('[data-open-order]').forEach(b=>{
    b.addEventListener('click',()=>openOrder(b.dataset.openOrder));
  });
}
...
function openOrder(orderNumber){
  ...
  orderDetail.innerHTML=`
    <div class="order-detail">
      ...
      <button type="button" class="dialog-x"
        data-close-order aria-label="Cerrar">×</button>

      <div class="detail-top">
        <div>
          <div class="detail-number">${escapeHTML(o.orderNumber)}</div>
      ...
      <div class="detail-totals">
        <div class="detail-total">${escapeHTML(formatTotals(o.totals))}</div>
      </div>

      <div class="detail-actions">
        <button type="button" data-send-self="${o.orderNumber}">
...
function deleteOrder(orderNumber){
  ...
  if(!confirm(`¿Eliminar el pedido ${o.orderNumber}?`))return;

  orders=orders.filter(x=>x.orderNumber!==orderNumber);
  saveOrders();
  orderDialog.close();
  renderAll();
...
renderAll(); const prefix=`PAN-${getISOYear(date)}-S${String(getISOWeek(date)).padStart(2,'0')}-`;
 let highest=0;
